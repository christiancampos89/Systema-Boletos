...
<body>
  <div id="div1" class="col-md-3   order-md-1">
    <form class="needs-validation" name="frmpesqfar" action="" method="POST">
      <p><label class="lblfield">PESQUISA 1:</label></p>
      <select class="form-control" name="txttipo" >
        <option value="nada">SELECIONE A SITUAÇAO DO BOLETO</option>
  ...
  <div id="div2" class="col-md-10   order-md-1">
    <p><label class="lblfield">PESQUISA 2:</label></p>
    <form class="needs-validation" name="frmpesqfar" action="" method="POST">
      <div class="row">
      ...
      <center><input type="submit" class="btn btn-success" value="BUSCAR"></center>
      <input type="hidden" name="pesquisar2" value="BUSCAR">
    </form>
  </div><br>
<?php
  if(isset($_POST['pesquisar1']) && $_POST['pesquisar1'] == "BUSCAR"){
    ...
    if($vsituacao == "nada" ){
      echo '<script> alert("Selecione a situação do Boleto!");</script>';
      exit;
    }elseif($vsituacao == "todos" AND empty($vdata_inicial) AND empty($vdata_final) AND empty($vtipo)){
      $sql = "SELECT f.*, b.*
                FROM fornecedores f 
    ...
    }elseif($vsituacao == "todos" AND !empty($vdata_inicial) || !empty($vdata_final) AND empty($vtipo)){
      if ($vdata_inicial == null){
          ...
          $sql = "SELECT f.*, b.*
                    FROM fornecedores f 
                    INNER JOIN boletos b ON f.id_fornecedor = b.id_fornecedor
                    WHERE data_vencimento BETWEEN '$vdata_inicial' AND '$vdata_final'
                    ORDER BY data_vencimento";
          $res = $mysqli->query($sql);
          $registros = mysqli_num_rows($res);
          $linha = $res->fetch_assoc();
      ...
      if ($vdata_inicial == null){
          ...
          $sql = "SELECT f.*, b.*
                    FROM fornecedores f 
                    INNER JOIN boletos b ON f.id_fornecedor = b.id_fornecedor
                    WHERE tpdespesa = '$vtipo' AND data_vencimento BETWEEN '$vdata_inicial' AND '$vdata_final'
                    ORDER BY data_vencimento";
          $res = $mysqli->query($sql);
          $registros = mysqli_num_rows($res);
          $linha = $res->fetch_assoc();
    ...
    //---------------------------------------------------------------------------------------------------------------------
      ...
      if ($vdata_inicial == null){
          ...
          $sql = "SELECT f.*, b.*
                    FROM fornecedores f 
                    INNER JOIN boletos b ON f.id_fornecedor = b.id_fornecedor
                    WHERE status = '$vsituacao' AND data_vencimento BETWEEN '$vdata_inicial' AND '$vdata_final'
                    ORDER BY data_vencimento";
          $res = $mysqli->query($sql);
          $registros = mysqli_num_rows($res);
          $linha = $res->fetch_assoc();
      ...
      if ($vdata_inicial == null){
          ...
          $sql = "SELECT f.*, b.*
                    FROM fornecedores f 
                    INNER JOIN boletos b ON f.id_fornecedor = b.id_fornecedor
                    WHERE status = '$vsituacao' AND data_vencimento BETWEEN '$vdata_inicial' AND '$vdata_final' AND tpdespesa = '$vtipo'
                    ORDER BY data_vencimento";
          $res = $mysqli->query($sql);
          $registros = mysqli_num_rows($res);
          $linha = $res->fetch_assoc();
...
?> 
  ...
  <?php }elseif(isset($_POST['pesquisar2']) && $_POST['pesquisar2'] == "BUSCAR"){
    $vsituacao = $_POST['txttipo'];
    $vfornecedor = $_POST['txtfornecedor'];
    $vtipo = (isset($_POST['tpmercadoria'])) ? $_POST['tpmercadoria'] : null;
    $vdata_inicial = (isset($_POST['txtdatainicial'])) ? $_POST['txtdatainicial'] : null;
    $vdata_final = (isset($_POST['txtdatafinal'])) ? $_POST['txtdatafinal'] : null;
    if($vsituacao == "nada" || $vfornecedor == "nada"){
      echo '<script> alert("Selecione a situação do Boleto e um Fornecedor!");</script>';
      exit;
    }elseif($vsituacao == "todos" AND empty($vdata_inicial) AND empty($vdata_final) AND empty($vtipo)){
      $sql = "SELECT f.*, b.*
                FROM fornecedores f 
                INNER JOIN boletos b ON f.id_fornecedor = b.id_fornecedor
                WHERE b.id_fornecedor = $vfornecedor
                ORDER BY data_vencimento";
      $res = $mysqli->query($sql);
      $registros = mysqli_num_rows($res);
      $linha = $res->fetch_assoc();
      ...
      if ($vdata_inicial == null){
          ...
          $sql = "SELECT f.*, b.*
                    FROM fornecedores f 
                    INNER JOIN boletos b ON f.id_fornecedor = b.id_fornecedor
                    WHERE b.id_fornecedor = $vfornecedor AND data_vencimento BETWEEN '$vdata_inicial' AND '$vdata_final'
                    ORDER BY data_vencimento";
          $res = $mysqli->query($sql);
          $registros = mysqli_num_rows($res);
          $linha = $res->fetch_assoc();
      ...
      if ($vdata_inicial == null){
          ...
          $sql = "SELECT f.*, b.*
                    FROM fornecedores f 
                    INNER JOIN boletos b ON f.id_fornecedor = b.id_fornecedor
                    WHERE b.id_fornecedor = $vfornecedor AND data_vencimento BETWEEN '$vdata_inicial' AND '$vdata_final' AND tpdespesa = '$vtipo'
                    ORDER BY data_vencimento";
          $res = $mysqli->query($sql);
          $registros = mysqli_num_rows($res);
          $linha = $res->fetch_assoc();
      ...
      $sql = "SELECT f.*, b.*
        FROM fornecedores f 
        INNER JOIN boletos b ON f.id_fornecedor = b.id_fornecedor
        WHERE b.id_fornecedor = $vfornecedor AND tpdespesa = '$vtipo'
        ORDER BY data_vencimento";
      $res = $mysqli->query($sql);
      $registros = mysqli_num_rows($res);
      $linha = $res->fetch_assoc();
    ...
    //---------------------------------------------------------------------------------------------------------------------
      ...
      $sql = "SELECT f.*, b.*
                FROM fornecedores f 
                INNER JOIN boletos b ON f.id_fornecedor = b.id_fornecedor
                WHERE b.id_fornecedor = $vfornecedor AND status = '$vsituacao'
                ORDER BY data_vencimento";
      $res = $mysqli->query($sql);
      $registros = mysqli_num_rows($res);
      $linha = $res->fetch_assoc();
      ...
      if ($vdata_inicial == null){
          ...
          $sql = "SELECT f.*, b.*
                    FROM fornecedores f 
                    INNER JOIN boletos b ON f.id_fornecedor = b.id_fornecedor
                    WHERE b.id_fornecedor = $vfornecedor AND status = '$vsituacao' AND data_vencimento BETWEEN '$vdata_inicial' AND '$vdata_final'
                    ORDER BY data_vencimento";
          $res = $mysqli->query($sql);
          $registros = mysqli_num_rows($res);
          $linha = $res->fetch_assoc();
      ...
      if ($vdata_inicial == null){
          ...
          $sql = "SELECT f.*, b.*
                    FROM fornecedores f 
                    INNER JOIN boletos b ON f.id_fornecedor = b.id_fornecedor
                    WHERE b.id_fornecedor = $vfornecedor AND status = '$vsituacao' AND data_vencimento BETWEEN '$vdata_inicial' AND '$vdata_final' AND tpdespesa = '$vtipo'
                    ORDER BY data_vencimento";
          $res = $mysqli->query($sql);
          $registros = mysqli_num_rows($res);
          $linha = $res->fetch_assoc();
      ...
      $sql = "SELECT f.*, b.*
        FROM fornecedores f 
        INNER JOIN boletos b ON f.id_fornecedor = b.id_fornecedor
        WHERE b.id_fornecedor = $vfornecedor AND status = '$vsituacao' AND tpdespesa = '$vtipo'
        ORDER BY data_vencimento";
      $res = $mysqli->query($sql);
      $registros = mysqli_num_rows($res);
      $linha = $res->fetch_assoc();
